...
export default function CategoryPicker() {
  ...
  return (
    <Section title="Content Type" desc="Pick category — sample data auto-fills" noPad>
        ...
        {CATEGORIES.map((cat) => {
          ...
          return (
            <button
              key={cat.id}
              type="button"
              onClick={() => applyPreset(cat.id as ContentCategory)}
              className={`flex shrink-0 items-center gap-2 rounded-[12px] border px-3 py-2.5 transition-all duration-200 ${
                active
