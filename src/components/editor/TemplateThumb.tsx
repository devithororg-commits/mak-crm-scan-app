...
const THUMBS: Record<TemplateId, ReactNode> = {
  ...
  'villa-prestige': (<div className="flex h-full flex-col justify-end bg-gradient-to-br from-amber-900 to-black p-2"><Bar w="w-8" h="h-1.5" color="bg-amber-200" /><Bar w="w-6" color="bg-white/40" /></div>),
  'penthouse-sky': (<div className="flex h-full flex-col justify-end bg-gradient-to-br from-slate-800 to-indigo-950 p-2"><Bar w="w-8" color="bg-white" /><Bar w="w-5" color="bg-white/40" /></div>),
  'commercial-hub': (<div className="flex h-full flex-col bg-white p-2"><div className="mb-1 h-2 bg-blue-600" /><Bar w="w-8" color="bg-slate-700" /><Bar w="w-5" color="bg-blue-500" /></div>),
  'plot-investment': (<div className="flex h-full flex-col justify-end bg-gradient-to-br from-emerald-800 to-emerald-950 p-2"><Bar w="w-7" color="bg-emerald-300" /><Bar w="w-5" color="bg-white/30" /></div>),
  'virtual-tour': (<div className="relative flex h-full items-center justify-center bg-gradient-to-br from-violet-700 to-indigo-900"><div className="h-5 w-5 rounded-full border-2 border-white" /></div>),
  'rental-premium': (<div className="flex h-full flex-col justify-end bg-gradient-to-br from-teal-700 to-slate-900 p-2"><Bar w="w-6" color="bg-teal-300" /><Bar w="w-4" color="bg-white/30" /></div>),
  'home-loan-pro': (<div className="flex h-full flex-col justify-end bg-gradient-to-br from-indigo-900 to-slate-950 p-2"><Bar w="w-8" color="bg-indigo-300" /><div className="mt-1 h-3 rounded bg-indigo-500/40" /></div>),
  'reel-cover': (<div className="flex h-full flex-col justify-end bg-gradient-to-br from-pink-500 to-orange-500 p-2"><Bar w="w-7" color="bg-white" /><Bar w="w-4" color="bg-white/50" /></div>),
  'linkedin-pro': (<div className="flex h-full flex-col bg-[#f3f2ef] p-2"><div className="mb-1 flex gap-1"><div className="h-3 w-3 rounded-full bg-blue-700" /><Bar w="w-5" color="bg-slate-600" /></div><div className="rounded bg-white p-1"><Bar w="w-6" color="bg-slate-700" /></div></div>),
  'product-launch-pro': (<div className="flex h-full flex-col justify-end bg-gradient-to-br from-violet-700 to-fuchsia-800 p-2"><Bar w="w-8" color="bg-white" /><div className="mt-1 grid grid-cols-3 gap-0.5">{[1,2,3].map(i=><div key={i} className="h-2 rounded bg-white/20"/>)}</div></div>),
  'startup-pitch': (<div className="flex h-full flex-col bg-white p-2"><div className="mb-1 h-2 bg-indigo-600" /><Bar w="w-8" color="bg-slate-700" /><div className="mt-1 grid grid-cols-2 gap-0.5">{[1,2,3,4].map(i=><div key={i} className="h-2 rounded bg-indigo-50"/>)}</div></div>),
  'podcast-cover': (<div className="flex h-full flex-col justify-end bg-gradient-to-br from-violet-600 to-indigo-800 p-2"><div className="mb-1 h-3 w-3 rounded-full bg-white/30" /><Bar w="w-7" color="bg-white" /></div>),
  'event-poster': (<div className="flex h-full flex-col justify-end bg-gradient-to-br from-violet-600 to-fuchsia-500 p-2"><Bar w="w-8" color="bg-white" /><div className="mt-1 h-2 rounded bg-white/20" /></div>),
  'flash-sale': (<div className="flex h-full flex-col items-center justify-center bg-gradient-to-br from-red-600 to-orange-500 p-2"><Bar w="w-6" color="bg-white" /><div className="mt-1 flex gap-0.5">{[1,2,3].map(i=><div key={i} className="h-3 w-3 rounded bg-white/25"/>)}</div></div>),
  'brand-manifesto': (<div className="flex h-full flex-col bg-slate-50 p-2"><Bar w="w-6" color="bg-slate-700" /><div className="mt-1 space-y-0.5">{[1,2,3].map(i=><Bar key={i} w="w-4/5" color="bg-slate-400"/>)}</div></div>),
  'quarterly-pro': (<div className="flex h-full flex-col justify-end bg-gradient-to-br from-slate-800 to-slate-950 p-2"><Bar w="w-5" color="bg-emerald-400" /><Bar w="w-8" color="bg-white/40" /></div>),
  'kpi-command': (<div className="grid h-full grid-cols-2 gap-px bg-white/10 p-px">{[1,2,3,4].map(i=><div key={i} className="bg-[#0B0F19] p-1"><Bar w="w-4" color="bg-blue-400"/></div>)}</div>),
  'case-study-pro': (<div className="flex h-full flex-col bg-white p-2"><div className="mb-1 h-0.5 bg-emerald-500" /><Bar w="w-7" color="bg-slate-700" /><div className="mt-1 grid grid-cols-3 gap-0.5">{[1,2,3].map(i=><div key={i} className="h-3 rounded bg-emerald-50"/>)}</div></div>),
  'whitepaper-pro': (<div className="flex h-full"><div className="w-2/5 bg-blue-800" /><div className="flex flex-1 flex-col justify-center p-1"><Bar w="w-5" color="bg-slate-600"/></div></div>),
  'investor-snapshot': (<div className="flex h-full flex-col justify-end bg-gradient-to-br from-slate-950 to-indigo-950 p-2"><Bar w="w-6" color="bg-indigo-300" /><div className="mt-1 grid grid-cols-2 gap-0.5">{[1,2,3,4].map(i=><div key={i} className="h-2 rounded bg-white/10"/>)}</div></div>),
  'art-deco-luxe': (<div className="relative flex h-full items-center justify-center bg-gradient-to-br from-amber-900 to-black p-2"><div className="absolute inset-2 rounded border border-amber-400/40" /><Bar w="w-6" color="bg-amber-300" /></div>),
  'swiss-poster': (<div className="grid h-full grid-cols-[1fr_3fr] bg-[#E8E4DE]"><div className="border-r-2 border-black" /><div className="flex flex-col justify-center p-1"><Bar w="w-full" h="h-2" color="bg-black"/></div></div>),
  'synthwave-retro': (<div className="relative flex h-full flex-col justify-end bg-gradient-to-b from-[#1a0a2e] to-pink-600 p-2"><Bar w="w-7" color="bg-white" /><div className="mt-1 h-2 bg-pink-400/30" style={{transform:'perspective(40px) rotateX(45deg)'}}/></div>),
  'minimal-luxe': (<div className="flex h-full flex-col justify-between bg-white p-2"><Bar w="w-4" color="bg-slate-300" /><Bar w="w-8" h="h-2" color="bg-slate-800" /><Bar w="w-3" color="bg-slate-400" /></div>),
  'holographic-pro': (<div className="flex h-full flex-col bg-[#0a0a12] p-1.5"><div className="h-0.5 rounded-full bg-gradient-to-r from-violet-500 via-pink-500 to-cyan-400" /><div className="mt-1 flex-1 rounded bg-white/10 p-1"><Bar w="w-5" color="bg-white"/></div></div>),
}
...
export const TEMPLATE_GROUP_MAP: Record<TemplateId, string> = {
  ...
  'villa-prestige': 'realestate',
  'penthouse-sky': 'realestate',
  'commercial-hub': 'realestate',
  'plot-investment': 'realestate',
  'virtual-tour': 'realestate',
  'rental-premium': 'realestate',
  'home-loan-pro': 'realestate',
  'reel-cover': 'social',
  'linkedin-pro': 'social',
  'product-launch-pro': 'business',
  'startup-pitch': 'business',
  'podcast-cover': 'social',
  'event-poster': 'social',
  'flash-sale': 'social',
  'brand-manifesto': 'social',
  'quarterly-pro': 'business',
  'kpi-command': 'business',
  'case-study-pro': 'business',
  'whitepaper-pro': 'business',
  'investor-snapshot': 'business',
  'art-deco-luxe': 'social',
  'swiss-poster': 'social',
  'synthwave-retro': 'social',
  'minimal-luxe': 'social',
  'holographic-pro': 'social',
}
