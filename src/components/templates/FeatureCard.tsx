...
export default function FeatureCard({ data }: { data: CreativeData }) {
  ...
  const header = (
    <div className="mb-6 flex shrink-0 items-center justify-between">
      ...
      />
      {data.badge && (
        <span className="shrink-0 rounded-full border border-white/30 px-4 py-1" style={t.subtitle}><HighlightText text={data.badge} data={data} /></span>
      )}
    </div>
  )
    ...
    >
      ...
      <div className="flex min-h-0 flex-1 flex-col justify-center px-10 py-10">
        <TemplateLayout data={data} header={header}>
          {data.eyebrow && <p className="mb-2 shrink-0 font-medium text-white/70" style={t.label}><HighlightText text={data.eyebrow} data={data} /></p>}
          <h2 className="mb-4 shrink-0 font-bold leading-tight" style={t.title}><HighlightText text={data.title} data={data} /></h2>
          <p className="shrink-0 leading-relaxed text-white/80" style={t.body}><HighlightText text={data.description || data.subtitle} data={data} /></p>
